...
import (
	...
	_ "ticket-service/docs" // generated by `swag init -g cmd/main.go`
...
// @description Send the access token from POST /auth/login as `Bearer <token>` in the Authorization header.
// @host localhost:8080
// @BasePath /
...
func main() {
	...
	// Swagger UI is opt-in (dev only): it describes the whole API, so it is
	// not exposed by default. Outside the internal-secret gate on purpose.
